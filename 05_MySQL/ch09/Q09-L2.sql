...
-- 생성한 인덱스를 실제로 적용하기 위해서 ANALYZE로 테이블을 분석하고 처리해줘야 함
ANALYZE TABLE usertbl;
-- Index_length가 0이었는데 분석 후 제대로 나오는 걸 볼 수 있다
SHOW TABLE STATUS LIKE 'usertbl';

-- 출생년도(birthYear)에 보조 인덱스 생성
-- 범수랑 시경이 생일 둘 다 1979이므로 중복값 때문에 인덱스 생성 에러 발생
...
-- 인덱스 삭제 (DROP INDEX 또는 ALTER TABLE 구문으로 둘다 가능)
