...
import org.scoula.weather.dto.WeatherItem;
...
@Controller // 메소드의 반환값을 같은 이름의 view를 반환한다 <-> @RestController : json을 반환한다, 더 많이 사용
@Slf4j
@RequestMapping("/weather")
@PropertySource({"classpath:/application.properties"}) // 어디서 리소스를 가져올지 명시
public class WeatherController {

    @Value("${weather.url}")  // application.properties 에서 키값으로 설정을 가져옴ㅣ
    private String URL;
    ...
    public String weather(Model model, @PathVariable(value="city", required = false) String city) {
        ...
        String url = UriComponentsBuilder.fromHttpUrl(URL)
                ...
                .queryParam("lang", "kr")
                .toUriString(); // 기본 URL에 새로운 쿼리스트링을 연결해서 새로운 주소 생성
        // 생성한 URL을 통해서 WeatherDTO로 결과값 반환 받아옴
